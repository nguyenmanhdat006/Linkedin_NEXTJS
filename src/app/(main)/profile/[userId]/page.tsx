...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!userSlug) {
        console.error("Không tìm thấy slug trên URL");
        setLoading(false);
        return;
      }
  ...
  return (
   <div className="min-h-screen bg-[#f3f2ef] font-sans pb-4 relative">
    ...
    <EditIntroDialog open={isEditIntroOpen} onOpenChange={setIsEditIntroOpen} profileData={profile} />
    ...
    <div className="container max-w-[1128px] mx-auto pt-6 px-0 md:px-4">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start mb-8">
       <div className="lg:col-span-8 flex flex-col gap-4">
        ...
        <ProfileHeader profile={profile} currentEducation={currentEducation} onEdit={() => setIsEditIntroOpen(true)}/>

        {/* Suggested + Analytics (kept inline since small) */}
            ...
            <div className="border border-gray-200 rounded-xl p-4 mt-4">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <Button variant="outline" className="rounded-full font-semibold border-gray-600 text-gray-600 hover:bg-gray-100 bg-transparent">Thêm tóm tắt</Button>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="rounded-xl border border-gray-300 shadow-sm bg-white">
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              <div className="flex gap-2 cursor-pointer hover:bg-gray-100 p-2 rounded-lg transition-colors"><Users className="h-6 w-6 text-gray-600 mt-1 shrink-0" /><div><div className="font-bold text-gray-900 hover:text-blue-700 hover:underline">0 lượt xem hồ sơ</div><p className="text-sm text-gray-600 leading-tight mt-1">Cập nhật hồ sơ của bạn để thu hút người xem.</p></div></div>
              <div className="flex gap-2 cursor-pointer hover:bg-gray-100 p-2 rounded-lg transition-colors"><Search className="h-6 w-6 text-gray-600 mt-1 shrink-0" /><div><div className="font-bold text-gray-900 hover:text-blue-700 hover:underline">0 lượt xuất hiện tìm kiếm</div><p className="text-sm text-gray-600 leading-tight mt-1">Xem tần suất bạn xuất hiện trong kết quả.</p></div></div>
            </div>
          </CardContent>
        </Card>
