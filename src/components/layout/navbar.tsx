      ...
      <div className="container mx-auto flex h-14 items-center justify-between px-4 max-w-7xl">
        ...
        <nav className="flex items-center gap-1 md:gap-6">
          ...
          <div className="hidden md:flex border-l border-gray-200 pl-6 ml-2 items-center gap-4">
            <NavItem icon={Grid} label="For Business" />
...
function NavItem({ icon: Icon, label, active }: { icon: any; label: string; active?: boolean }) {
  return (
    <Link
      href="#"
      className={`group flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-lg transition-all duration-200 ${
        active 
    ...
    >
      <Icon 
        className={`h-6 w-6 transition-all duration-200 ${
          active ? "stroke-[2.5] fill-current" : "group-hover:scale-110"
        }`} 
      />
      ...
      {active && <div className="absolute bottom-0 h-0.5 w-12 bg-gray-900 rounded-full mt-1" />}
    </Link>
  )
}